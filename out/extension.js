...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('forgevsc.createConfig', async () => {
        ...
        const defaultConfig = {
            ...
            "urls": [
                "https://github.com/tryforge/forgescript/tree/dev/metadata/functions.json"
            ]
        };
    ...
    context.subscriptions.push(vscode.commands.registerCommand('forgevsc.updateLSP', async () => {
        ...
        let serverPath;
        const customPath = context.globalState.get('customBinaryPath');
...
async function startClient(context) {
    ...
    if (!fs.existsSync(serverPath)) {
        const userChoice = await vscode.window.showInformationMessage(`ForgeLSP binary not found. Download ${serverBinaryName}?`, 'Download', 'Cancel');
        ...
        if (updateAvailable) {
            const userChoice = await vscode.window.showInformationMessage('A new version of ForgeLSP is available. Update now?', 'Update', 'Skip');
    ...
    client = new node_1.LanguageClient('forgeLSP', 'Forge Language Server', serverOptions, clientOptions);
...
function getServerBinaryName() {
    ...
    if (platform === 'linux') {
        if (arch === 'x64')
            return 'forgevsc-linux-x86_64';
        if (arch === 'arm64')
            return 'forgevsc-linux-aarch64';
    }
    else if (platform === 'darwin') {
        if (arch === 'x64')
            return 'forgevsc-macos-x86_64';
        if (arch === 'arm64')
            return 'forgevsc-macos-aarch64';
    }
    else if (platform === 'win32') {
        if (arch === 'x64')
            return 'forgevsc-windows-x86_64.exe';
    }
    return null;
}
// ============================================================================
 ...
 */
async function getLatestReleaseInfo(binaryName) {
    try {
        const response = await axios_1.default.get('https://api.github.com/repos/zack-911/forgelsp/releases/tags/master');
 ...
 */
function readMetadata(binaryPath) {
 ...
 */
function writeMetadata(binaryPath, metadata) {
    const metadataPath = getMetadataPath(binaryPath);
    try {
 ...
 */
async function shouldUpdate(binaryPath, binaryName) {
